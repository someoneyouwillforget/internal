...
function Elements.CreateToggle(TabPage, TData, Theme)
    ...
    local SwitchBar = Instance.new("Frame", Toggle)
    ...
    Toggle.MouseButton1Click:Connect(function() state = not state; update(false) end)
    update(true)
end
...
function Elements.CreateSlider(TabPage, SData, Theme)
    ...
    Title.Font = Theme.TextFont; Title.TextSize = 14; Title.TextXAlignment = "Left"

    local ValueLabel = Instance.new("TextLabel", SliderFrame)
    ValueLabel.Size = UDim2.new(1, -15, 0, 25)
    ValueLabel.BackgroundTransparency = 1; ValueLabel.TextColor3 = Theme.TextColor
    ValueLabel.Font = Theme.TextFont; ValueLabel.TextSize = 14; ValueLabel.TextXAlignment = "Right"
...
function Elements.CreateDropdown(TabPage, DData, Theme)
    ...
    MainBtn.TextColor3 = Theme.TextColor; MainBtn.Font = Theme.TextFont
    MainBtn.TextSize = 14; MainBtn.TextXAlignment = "Left"
    ...
    local layout = Instance.new("UIListLayout", ItemHolder)
    layout.Padding = UDim.new(0, 4)

    MainBtn.MouseButton1Click:Connect(function()
        Dropped = not Dropped
        local targetSize = Dropped and UDim2.new(1, 0, 0, layout.AbsoluteContentSize.Y + 45) or UDim2.new(1, 0, 0, 36)
        TweenService:Create(DropFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Size = targetSize}):Play()
    end)
    ...
    for _, option in pairs(DData.Options) do
        ...
        OptBtn.Text = option; OptBtn.TextColor3 = Theme.TextColor
        OptBtn.Font = Theme.TextFont; OptBtn.TextSize = 13
        Instance.new("UICorner", OptBtn).CornerRadius = UDim.new(0, 6)
        
        OptBtn.MouseButton1Click:Connect(function()
            MainBtn.Text = "  " .. DData.Name .. " : " .. option
            Dropped = false
            TweenService:Create(DropFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Size = UDim2.new(1, 0, 0, 36)}):Play()
